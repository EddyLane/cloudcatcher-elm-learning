...
const initiateElmApp = getStorage => {
		...
		db.get(STATE_KEY)
            ...
            .catch((e) => {
                if (e.status === 409) {
                    ...
                    db.put(Object.assign({}, model, { _id: STATE_KEY }));
                }
                else {
	...
	imageDb.lru.info().then(function (info) {
		...
		console.debug(info);
		Object.keys(info.items).forEach(url => retrieveImage(url).then(data => app.ports.images.send(data)));

	});

	app.ports.incomingImages.subscribe(images => getAllImages(images, app));
...
const retrieveImage = uri => new Promise((resolve) => {
